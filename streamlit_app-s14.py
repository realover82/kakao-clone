...
def analyze_data(df, date_col_name, jig_col_name):
    ...
    # 지그(PC) 컬럼에 데이터가 없는 경우 'SNumber'를 대체 컬럼으로 사용 (PCB 탭의 경우)
    used_jig_col_name = jig_col_name
    if jig_col_name == 'PcbMaxIrPwr' and (jig_col_name not in df.columns or df[jig_col_name].isnull().all()):
        used_jig_col_name = 'SNumber'
