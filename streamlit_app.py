...
def display_analysis_result(analysis_key, table_name, date_col_name, selected_jig=None):
    ...
    with col1:
        if st.button("꺾은선 그래프 보기"):
            st.session_state.show_line_chart = not st.session_state.show_line_chart
        if st.session_state.show_line_chart:
            st.line_chart(chart_data)
    with col2:
        if st.button("막대 그래프 보기"):
            st.session_state.show_bar_chart = not st.session_state.show_bar_chart
...
def main():
    ...
    if not st.session_state.snumber_search_results.empty:
